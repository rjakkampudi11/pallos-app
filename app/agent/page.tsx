...
const navGroups: { label: string; items: { id: View; label: string; icon: typeof House }[] }[] = [
  { label: "Scan", items: [{ id: "overview", label: "Home", icon: House }, { id: "projects", label: "Projects", icon: Folder }, { id: "findings", label: "Findings", icon: WarningCircle }] },
  ...
  return (
    <main className={`agent-app theme-${appearance.theme} accent-${appearance.accent} density-${appearance.density} ${appearance.reducedMotion ? "reduce-motion" : ""}`}>
      <aside className={`agent-side ${mobileNav ? "mobile-open" : ""}`}>
        <div className="agent-side-head"><button className="agent-brand" onClick={() => openView("overview")}><span className="agent-brand-dot" />Pallos</button><button className="close-mobile" aria-label="Close navigation" onClick={() => setMobileNav(false)}><X /></button></div>
      ...
      <section className="agent-main">
        ...
        <div className="agent-content">
          ...
          {view === "connections" && <><GitHubWorkspace notify={setToast} /><SupabaseInspection notify={setToast} /><p className="connection-note">GitHub checks repository code. The optional Supabase connection checks database access settings. Other integrations are not available yet.</p></>}
...
function Overview({ runScan, running, lastRun, openView, selectFinding }: { runScan: () => void; running: boolean; lastRun: string; openView: (view: View) => void; selectFinding: (index: number) => void }) {
  return <>
    ...
    <section className="overview-status">
      <div><span><i className="status-light" />WORKSPACE READY</span><strong>Repository scanning is ready</strong></div>
...
function TesterFeedback({ notify }: { notify: (value: string) => void }) {
  ...
  return <form className="workspace-card tester-feedback" onSubmit={submit}><div className="feedback-intro"><span>TESTER FEEDBACK</span><h2>Tell us what worked—and what did not.</h2><p>This takes about two minutes. Your answers are saved in Pallos and emailed to our team.</p></div><label>How useful was Pallos?<select name="usefulness" required defaultValue=""><option value="" disabled>Choose 1–10</option>{Array.from({ length: 10 }, (_, index) => <option key={index + 1}>{index + 1}</option>)}</select></label><label>Was setup clear?<select name="setupClarity" required defaultValue=""><option value="" disabled>Choose one</option><option>Very clear</option><option>Mostly clear</option><option>Confusing</option></select></label><label>Were the security findings clear?<select name="detectionClarity" required defaultValue=""><option value="" disabled>Choose one</option><option>Very clear</option><option>Mostly clear</option><option>Confusing</option></select></label><label>What confused you?<textarea name="confusingText" rows={3} /></label><label>What feature was missing?<textarea name="missingFeature" rows={3} /></label><label>Would you use Pallos again?<select name="reuseIntent" required defaultValue=""><option value="" disabled>Choose one</option><option>Yes</option><option>Maybe</option><option>No</option></select></label><label>Would you pay for recurring repository scans?<select name="willingnessToPay" required defaultValue=""><option value="" disabled>Choose one</option><option>Yes, now</option><option>Maybe after more features</option><option>No</option></select></label><label className="feedback-consent"><input name="contactPermission" type="checkbox" />Pallos may email me one follow-up question.</label><button className="run-button" disabled={sending}>{sending ? "Sending…" : "Send tester feedback"}</button></form>;
}
