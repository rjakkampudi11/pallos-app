...
export function SecuritySettings({ notify }: { notify: (message: string) => void }) {
  ...
  return <div className="settings-grid security-settings-grid">
    <section className="workspace-card settings-card"><div className="card-head"><div><span>PROTECTION</span><h2>Security controls</h2></div><ShieldCheck /></div><div className="security-control-list"><div><strong>Verified email</strong><span>New accounts must confirm their inbox before accessing a workspace.</span></div><div><strong>Abuse limits</strong><span>Login, signup, monitor checks, and repository scans have server-enforced limits.</span></div><div><strong>Private storage</strong><span>Account rows use ownership policies; monitor headers are encrypted before storage.</span></div></div><Link className="outline-action settings-action" href="https://pallosagent.com/security" target="_blank">Read the public security page <ArrowSquareOut /></Link></section>
