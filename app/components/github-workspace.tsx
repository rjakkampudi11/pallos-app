...
export function GitHubWorkspace({ mode = "connections", notify }: { mode?: "connections" | "projects" | "findings"; notify?: (message: string) => void }) {
  ...
  if (error && repositories.length === 0) return <section className="workspace-card github-empty" role="alert"><WarningCircle /><h2>Couldn’t load your GitHub connection.</h2><p>Your connection may still be active. Try loading it again.</p><button className="outline-action" onClick={() => void load()}><ArrowClockwise />Try again</button></section>;
