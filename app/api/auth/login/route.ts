...
export async function POST(request: Request) {
  ...
  let input: { email?: string; password?: string; rememberMe?: boolean };
  ...
  return setAuthCookies(NextResponse.json({ user: { id: data.user.id, email: data.user.email, displayName: data.user.user_metadata.display_name || email.split("@")[0] } }), data.session, input.rememberMe === true);
}
