    ...
    <header className="site-header">
      <nav className="nav shell" aria-label="Main navigation">
        ...
        <div className={`navlinks ${menuOpen ? "open" : ""}`}>
          <a href="#checks" onClick={() => setMenuOpen(false)}>What it checks</a><a href="#how" onClick={() => setMenuOpen(false)}>How it works</a><a href="#demo" onClick={() => setMenuOpen(false)}>Demo</a><a href="#faq" onClick={() => setMenuOpen(false)}>FAQ</a><a href="#contact" onClick={() => setMenuOpen(false)}>Contact</a>
        </div>
        <a className="button small nav-cta" href="#waitlist">Join waitlist <ArrowRight weight="bold" /></a>
    ...
    <div id="main-content">
      <section className="hero section shell">
        <div className="hero-copy-wrap reveal">
          ...
          <div className="hero-actions"><a className="button" href="#waitlist">Join the private beta <ArrowRight weight="bold" /></a><a className="ghost-button" href="#demo">See a scan snapshot</a></div>
      ...
      <section className="section waitlist-section" id="waitlist"><div className="shell waitlist-grid"><div className="waitlist-copy"><div className="eyebrow">PRIVATE BETA</div><h2>Building something real with AI?</h2><p>Join the waitlist and tell us what you are making. Early access will focus on founders using Next.js, JavaScript, TypeScript, and Supabase.</p><div className="privacy-note"><Check weight="bold" />One useful email when access opens. No list selling.</div></div><div className="form-card">{submitState === "success" ? <div className="success"><span><Check weight="bold" /></span><h3>You are on the list.</h3><p>{submitMessage}</p><button className="text-link" onClick={() => setSubmitState("idle")}>Add another person</button></div> : <form onSubmit={submitWaitlist}><div className="form-row"><label>First name<input name="firstName" required placeholder="Your name" /></label><label>Email address<input name="email" required type="email" placeholder="you@company.com" /></label></div><div className="form-row"><label>Your role<select name="role" required defaultValue=""><option value="" disabled>Select one</option><option>Founder</option><option>Freelancer</option><option>Agency</option><option>Developer</option><option>Small business owner</option></select></label><label>Main AI coding tool<select name="tool" required defaultValue="" onChange={(event) => setOtherTool(event.target.value === "Other")}><option value="" disabled>Select one</option><option>Lovable</option><option>Replit</option><option>Bolt</option><option>v0</option><option>Cursor</option><option>Claude Code</option><option>Codex</option><option>Other</option></select></label></div>{otherTool && <label className="conditional-field">What other AI coding platform do you use?<input name="otherTool" required placeholder="Enter the platform name" autoFocus /></label>}<div className="form-row"><label>Biggest concern<select name="concern" required defaultValue=""><option value="" disabled>Select one</option><option>Exposed keys</option><option>Customer data</option><option>Database permissions</option><option>Unsafe admin access</option><option>AI-generated changes</option><option>Unexpected API costs</option></select></label><label>Project stage<select name="stage" required defaultValue=""><option value="" disabled>Select one</option><option>Idea</option><option>Building</option><option>Testing</option><option>Already launched</option></select></label></div><label>What are you building?<textarea name="building" placeholder="A short description helps us prioritize the right checks." /></label><label className="honeypot" aria-hidden="true">Company website<input name="website" tabIndex={-1} autoComplete="off" /></label><label className="consent"><input name="consent" value="yes" type="checkbox" required />I agree to receive updates about the Pallos private beta.</label><button className="button full" disabled={submitState === "sending"}>{submitState === "sending" ? "Joining…" : "Join the waitlist"}<ArrowRight weight="bold" /></button>{submitState === "error" && <p className="form-message error" role="alert">{submitMessage}</p>}</form>}</div></div></section>
    </div>

    <footer className="footer" id="contact"><div className="shell footer-top"><div><Link className="brand" href="#top"><span className="brand-dot" />Pallos Agent</Link><p>A clearer second pass for people building apps with AI.</p></div><div className="footer-contact"><span>CONTACT</span><a href="mailto:pallosagent@gmail.com"><EnvelopeSimple />pallosagent@gmail.com</a><button type="button" onClick={() => navigator.clipboard.writeText("pallosagent")}><DiscordLogo />Discord: pallosagent</button></div><div className="social-directory">{socialAccounts.map(({ label, handle, href, icon: Icon }) => <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={`${label}: ${handle}`}><Icon /><span><small>{label}</small>{handle}</span><ArrowRight /></a>)}</div></div><div className="shell footer-bottom"><span>© 2026 Pallos Agent</span><div><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link></div></div></footer>
