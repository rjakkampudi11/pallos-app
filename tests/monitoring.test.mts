...
import { compareResponse, fetchEndpoint, type EndpointSnapshot, type JsonValue } from "../lib/monitoring.ts";
...
test("an unchanged response contract is healthy", () => {
  assert.deepEqual(compareResponse(baseline, snapshot({ ...baseline, updated_at: "later" })), []);
});

test("missing fields and type changes are serious", () => {
  const changes = compareResponse(baseline, snapshot({ user_id: 1042, full_name: "Maya Chen", subscription_status: "active", updated_at: "later" }));
...
test("new fields are saved but do not create a serious incident", () => {
  const changes = compareResponse(baseline, snapshot({ ...baseline, account_region: "us-east" }));
...
test("HTTP errors are serious", () => {
  const changes = compareResponse(baseline, { ok: false, statusCode: 503, body: { error: "down" }, durationMs: 15, errorMessage: "HTTP 503" });
...
test("the Pallos healthy and fault URLs produce a serious contract change", async () => {
  ...
  assert.ok(compareResponse(healthy.body!, fault).some((change) => change.serious));
});
